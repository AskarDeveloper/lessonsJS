...
let appData = {
  ...
  chooseOptExpenses: function () {
    for (let i = 0; i < 3; i++) {
      let opt = prompt("Введите статью необъязательных расходов?", "");
  ...
  chooseIncome: function () {
    for (let i = 1; i < 2; i++) {
      let items = prompt("Что принесет дополнительный доxод? (Перечислите через запятую)", "");
      if ((typeof (items)) === 'string' && (typeof (items)) != null && items != '' && isNaN(items)) {
        appData.income[i] = items.split(', ');
        appData.income[i].push(prompt('Может что-то еще?', ''));
        appData.income.sort();
      } else {
        i = i - 1;
      }
